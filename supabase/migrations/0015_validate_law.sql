-- Phase 6.2 — content pipeline validation gate.
-- public.validate_law(law_slug) checks the structural invariants the app
-- relies on for a single law. The extractor runs this after each load and
-- fails loudly on any row where ok = false.
